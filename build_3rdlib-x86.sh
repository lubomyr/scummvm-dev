...
cd libpng-1.6.21
make distclean
./configure --host=x86 --prefix=$LIBPATH
make
make install
cd ..
...
cd libmad-0.15.1b
make distclean
./configure --host=x86 --prefix=$LIBPATH --enable-speed --enable-fpm=intel --disable-aso
make
make install
cd ..
...
cd Tremor
make distclean
./autogen.sh --host=x86 --prefix=$LIBPATH --enable-low-accuracy --enable-64kHz-limit
make
make install
cd ..
...
cd flac-1.2.1
make distclean
./configure --host=x86 --prefix=$LIBPATH --disable-largefile --disable-thorough-tests --disable-doxygen-docs --disable-xmms-plugin --disable-cpplibs --disable-ogg --disable-oggtest --disable-rpath
make
make install
cd ..
...
cd libtheora-1.1.1
make distclean
./configure --host=x86 --prefix=$LIBPATH --with-ogg=$LIBPATH --disable-examples
make
make install
cd ..
